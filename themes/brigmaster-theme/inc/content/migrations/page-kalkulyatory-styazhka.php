...
{
    private const MIGRATION_VERSION = 'screed-v7';
    ...
    public static function migrate_screed_page(int $page_id): array
    {
        ...
        if (defined('RANK_MATH_VERSION')) {
            update_post_meta($page_id, 'rank_math_title', 'Калькулятор стяжки пола — расчёт смеси онлайн');
            update_post_meta($page_id, 'rank_math_description', 'Рассчитайте расход сухой смеси или цемента и песка для стяжки пола. Объём раствора по площади и толщине, нормы СП 29.13330, пример расчёта.');
        }
        ...
        $blocks = [
            ...
            Constructly_Migration_Helpers::block('constructly/context-intro', [
                ...
                'body' => '<p>Стяжку пола используют для выравнивания основания под финишное покрытие, повышения жёсткости конструкции пола и размещения систем тёплого пола. Она актуальна в новостройках с неровными плитами перекрытия, при капитальном ремонте с заменой покрытия, а также в помещениях с перепадами высот более 5 мм. Мокрая и полусухая стяжки применяются в жилых и коммерческих помещениях; наливной состав — там, где требуется минимальная толщина слоя.</p>'
                    . '<p>Главное при расчёте — определить количество сухой смеси или соотношение цемента и песка. Готовые смеси (М150, М200 по ГОСТ 31358-2019) удобны для небольших объёмов: расход составляет 19 кг/м² на каждые 10 мм толщины при плотности смеси 1900 кг/м³. Самомесный цементно-песчаный раствор (ЦПС) выгоднее при объёме от 1 м³ — стандартная пропорция 1:3 (цемент М400 : песок) даёт раствор марки М150 по ГОСТ 28013-98 и на 20–40 % дешевле готовой смеси.</p>'
                    . '<p>Стяжка пола смета включает стоимость раствора или сухой смеси, армирования (при необходимости), маяков и демпферной ленты. Калькулятор ниже считает материальную часть по вашим параметрам; трудозатраты зависят от выбранной технологии — мокрая, полусухая или наливная стяжка.</p>',
            ]),
            Constructly_Migration_Helpers::block('constructly/calculator-estimator', [
                ...
                'infoBody' => '<p>Калькулятор рассчитывает материалы для стяжки пола: объём раствора и количество смеси по площади помещения и толщине слоя.</p>'
                    ...
                    . '<tr><td><span class="bm-tooltip" data-tooltip="Для жилых помещений: сварная сетка Вр-I или А400 Ø6 мм, шаг 150×150 мм — расход ≈ 3,1 кг/м² (один слой, без запаса) / ≈ 3,4 кг/м² с запасом 10 %; ГОСТ 23279-2012. Линейная масса Ø6 мм: 0,222 кг/м." tabindex="0">Арматурная сетка (опционально)</span></td><td>≈ 3,4 кг/м² (Ø6 мм, шаг 150×150 мм, запас 10 %)</td></tr>'
                    . '</tbody>'
                    . '</table>'
            ...
            Constructly_Migration_Helpers::block('constructly/faq', [
                ...
                'items' => [
                    [
                        'question' => 'Какую толщину стяжки выбрать?',
                        'answer'   => 'По СП 29.13330.2011 минимальная толщина цементно-песчаной стяжки по жёсткому основанию — 20 мм, по тепло- или звукоизоляционному слою — 40 мм. При тёплом поле (трубы водяного контура) норма — 45 мм над трубой: для трубы диаметром 16 мм итого получается 61 мм от верха утеплителя. На практике в жилых помещениях чаще выбирают 40–50 мм — этого достаточно для укладки плитки или ламината без риска трещин. Подробнее: <a href="/baza-znaniy/poly/tolshchina-styazhki-pola/">Толщина стяжки пола: нормы и таблица</a>.',
                    ],
                    [
                        'question' => 'Сколько сухой смеси нужно на 1 м²?',
                        'answer'   => 'Расход готовой сухой смеси составляет 19 кг/м² на каждые 10 мм толщины при плотности смеси 1900 кг/м³ (средняя по данным производителей, диапазон 18–22 кг/м²/10 мм). На стяжку 40 мм / 20 м² потребуется: 20 × 4 × 19 = 1520 кг — то есть 61 мешок по 25 кг (округление вверх). Рекомендуется купить на 10 % больше расчётного — на потери при замесе и неровность основания. Расчёт автоматически выполняется в форме выше.',
                    ],
                    [
                        'question' => 'Готовая смесь или цемент с песком — что выгоднее?',
                        'answer'   => 'Готовая смесь удобна: точный состав, нет риска ошибиться с пропорцией, меньше времени на замес. Самомесный цементно-песчаный раствор (ЦПС) дешевле на 20–40 % при объёме от 1 м³ — именно с такого порога удобно работать с мешками цемента и заказной машиной песка. Калькулятор считает оба варианта: переключите тип материала в форме выше и сравните итог.',
                    ],
                    [
                        'question' => 'Какая пропорция ЦПС для стяжки?',
                        'answer'   => 'Для жилых помещений чаще используют пропорцию 1:3 (цемент М400 : песок по объёму) — она даёт раствор марки М150 по ГОСТ 28013-98, что достаточно для большинства покрытий. Пропорция 1:4 даёт марку около М100 и подходит для подготовительных слоёв без механических нагрузок. Пропорцию 1:2,5 (марка М200) применяют в помещениях с повышенной нагрузкой: гараж, склад. Водоцементное отношение (В/Ц) для мокрой ЦПС — 0,5–0,55; избыток воды снижает прочность.',
                    ],
                    [
                        'question' => 'Нужно ли армировать стяжку в квартире?',
                        'answer'   => 'В жилых помещениях при толщине стяжки до 50–60 мм по жёсткому основанию армирование не обязательно. По СП 29.13330.2011 армирование требуется при устройстве стяжки по сжимаемому основанию (утеплитель, звукоизоляция) независимо от толщины. При тёплом поле сетка рекомендуется для фиксации труб и распределения нагрузки. Для больших площадей (более 30 м²) и толщины от 60 мм используют сварную сетку Ø6 мм, шаг 150×150 мм; альтернатива — фиброволокно (0,6–0,9 кг/м³ раствора).',
                    ],
                    [
                        'question' => 'Расход арматуры на 1 м² стяжки — как считать?',
                        'answer'   => 'Для сварной сетки Вр-I или А400 Ø6 мм с шагом 150×150 мм расход составляет около 3,1 кг/м² (один слой, без запаса) или 3,4 кг/м² с запасом 10 % на нахлёст (ГОСТ 23279-2012; линейная масса Ø6 мм = 0,222 кг/м). Нахлёст при стыковке карт — не менее одной ячейки (150 мм). Калькулятор рассчитывает сетку опционально: задайте диаметр, шаг и количество слоёв в дополнительных параметрах.',
                    ],
                    [
                        'question' => 'Что такое стяжка пола смета и что в неё входит?',
                        'answer'   => 'Смета на стяжку включает материалы и работы. Материальная часть: сухая смесь или цемент с песком, арматурная сетка (при необходимости), маяки, демпферная лента по периметру, гидроизоляционная плёнка. Трудозатраты зависят от технологии: мокрая стяжка — дешевле, полусухая — быстрее (можно ходить через 12–24 ч). Калькулятор даёт материальную часть; стоимость работ уточняйте по региональным расценкам.',
                    ],
                    [
                        'question' => 'Учитывается ли усадка раствора при расчёте?',
                        'answer'   => 'Да. Для самомесного цементно-песчаного раствора сухой объём компонентов определяется с коэффициентом уплотнения 1,33: это стандартная поправка, учитывающая, что из 1 м³ сухих компонентов получается около 0,75 м³ готового раствора. Коэффициент применяется согласно практике нормирования (ГОСТ 28013-98) и заложен в расчёт калькулятора. Для готовой сухой смеси расход указывается производителем в кг/м²/10 мм и уже учитывает уплотнение.',
                    ],
                ],
            ]),
